...
The same two lemmas over a base in `CUBE` (e.g. `𝕀`), which lives in a separate
universe from `U`.

```rzk
#def transport-section-eq-at-cube
  ( X : CUBE)
  ( E : X → U)
  ( x0 : X)
  ( a : E x0)
...
#def transport-section-eq-at-cancel-cube
  ( X : CUBE)
  ( E : X → U)
  ( x0 : X)
  ( a : E x0)
  ...
  :=
    ...
    in
      concat
        ...
        ( concat (E x0) (ψ x0) (φ x0) a (rev (E x0) (φ x0) (ψ x0) apb) α)
        q
        ( transport-section-eq-at-cube X E x0 a φ ψ b α)
        ( concat
            ( ψ x0 = a)
            ( concat (E x0) (ψ x0) (φ x0) a
                ( rev (E x0) (φ x0) (ψ x0) apb)
