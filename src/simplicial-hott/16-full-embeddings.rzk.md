...
#def eq-dhom-refl-is-locally-contr-extext uses (extext)
  ...
  : ( t : Δ¹) → (f t =_{ A t } g t) [t ≡ 0₂ ↦ refl , t ≡ 1₂ ↦ refl]
  :=
  let f-eq-g := \ (t : Δ¹) → f t =_{ A t } g t in
  let f-eq-g-canon
    := \ (t : Δ¹) → all-elements-equal-is-contr (A t) (is-contr-A t) (f t) (g t)
  in
  first
  ( equiv-extension-homotopy-constraint 2 Δ¹ ∂Δ¹ f-eq-g
    ( \ t → recOR(t ≡ 0₂ ↦ refl , t ≡ 1₂ ↦ refl)))
  ( f-eq-g-canon
  , naiveextext-extext extext 2 ∂Δ¹ (\ _ → ⊥) f-eq-g
    ( \ _ → recBOT)
    ( \ t → recOR(t ≡ 0₂ ↦ refl , t ≡ 1₂ ↦ refl))
    ( f-eq-g-canon)
    ( \ t → rev (f t =_{ A t } g t)
      ( f-eq-g-canon t)
      ( recOR (t ≡ 0₂ ↦ refl , t ≡ 1₂ ↦ refl))
      ( path-eq-path-through-center-is-contr (A t) (is-contr-A t) (f t) (g t)
        ( recOR(t ≡ 0₂ ↦ refl , t ≡ 1₂ ↦ refl)))))
```
...
final objects are detected by full embeddings:
...
#def is-full-emb-total-type-hom-iso uses (extext)
  ...
  ( first
    ( axiom-choice-dhom B v w (\ b → Iso A (first is-rezk-A) (x b) (y b)) f g))
  ( second
    ( axiom-choice-dhom B v w (\ b → Iso A (first is-rezk-A) (x b) (y b)) f g))
  ( \ (h , γ) → (h , \ t → first (γ t)))
  ( is-equiv-total-is-equiv-fiberwise (hom B v w)
    ...
    ( \ h →
      ...
      ( f) (g)))
  ( first
    ( inv-axiom-choice-dhom B v w (\ b → hom A (x b) (y b)) (first f) (first g)))
  ( second
    ( inv-axiom-choice-dhom B v w (\ b → hom A (x b) (y b)) (first f) (first g)))
```
